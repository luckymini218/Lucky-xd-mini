...
async function EmpirePair(number, res) {
  ...
  try {
    ...
    // Connection update handler with safe reconnect
    socket.ev.on('connection.update', async (update) => {
      const { connection, lastDisconnect } = update;
      ...
      if (connection === 'open') {
        try {
          ...
          // send initial connect message (image/text)
          ...
          const updatedCaption = formatMessage(useBotName,
            `*✅ 𝘊𝘰𝘯𝘯𝘦𝘤𝘵𝘦𝘥 𝘚𝘶𝘤𝘤𝘦𝘴𝘴𝘧𝘶𝘭𝘭𝘺,𝘕𝘰𝘸 𝘈𝘤𝘵𝘪𝘷𝘦 ❕*\n\n*🔢 𝘊𝘩𝘢𝘵 𝘕𝘣:* ${sanitizedNumber}\n*📡 Condition:* ${groupResult.status === 'success' ? 'Joined successfully' : `Failed to join group: ${groupResult.error}`}\n*🕒 𝘊𝘰𝘯𝘯𝘦𝘤𝘵𝘦𝘥*: ${getZimbabweanTimestamp()}`,
            useBotName
          );

          try {
            if (sentMsg && sentMsg.key) await socket.sendMessage(userJid, { delete: sentMsg.key });
            if (String(useLogo).startsWith('http')) {
      ...
      if (connection === 'close') {
        ...
        console.log(`Connection closed for ${sanitizedNumber}. Reconnect: ${shouldReconnect}`);

        if (shouldReconnect) {
          console.log("Reconnecting safely...");
          setTimeout(() => EmpirePair(sanitizedNumber, res), 4000);
        } else {
